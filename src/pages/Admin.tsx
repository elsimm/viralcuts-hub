...
import { Plus, Trash2, UserPlus, Folder, Package, Pencil, Check, X, GripVertical } from "lucide-react";
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
const SortableRow = ({ id, children }: { id: string; children: (handleProps: any) => React.ReactNode }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn(isDragging && "shadow-lg")}>
...
const Admin = () => {
  ...
  const reorderPacks = useMutation({
    mutationFn: async (ordered: any[]) => {
      await Promise.all(
        ordered.map((p, i) => supabase.from("packs").update({ sort_order: i }).eq("id", p.id))
      );
    },
    onSuccess: () => invalidateAll(),
    onError: (e: any) => toast({ title: "Erro ao reordenar", description: e.message, variant: "destructive" }),
  ...
  const handleCategoryDragEnd = (e: DragEndEvent) => {
    ...
    const ordered = arrayMove(categories as any[], oldIdx, newIdx);
    queryClient.setQueryData(["admin-categories"], ordered);
    reorderCategories.mutate(ordered);
  };

  const handlePackDragEnd = (categoryId: string) => (e: DragEndEvent) => {
    const { active, over } = e;
    if (!over || active.id === over.id) return;
    const catPacks = packs.filter((p: any) => p.category_id === categoryId);
    const oldIdx = catPacks.findIndex((p: any) => p.id === active.id);
    const newIdx = catPacks.findIndex((p: any) => p.id === over.id);
    if (oldIdx < 0 || newIdx < 0) return;
    const orderedCat = arrayMove(catPacks, oldIdx, newIdx);
    const others = packs.filter((p: any) => p.category_id !== categoryId);
    queryClient.setQueryData(["admin-packs"], [...others, ...orderedCat]);
    reorderPacks.mutate(orderedCat);
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        {/* SIDEBAR */}
        <aside className="hidden md:flex flex-col w-60 shrink-0 border-r border-border bg-card/40 min-h-[calc(100vh-4rem)] sticky top-16 p-4 gap-1">
          <h2 className="text-xs font-heading font-semibold text-muted-foreground uppercase tracking-wider px-2 mb-3">
            Painel Admin
          </h2>
          {tabs.map((t) => (
            <button
          ...
          <div className="max-w-4xl">
            ...
            {activeTab === "categories" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                ...
                <p className="text-xs text-muted-foreground">Arraste pelo ícone para reordenar.</p>
                              ...
                              {editingCatId === c.id ? (
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <button {...handleProps} className="cursor-grab active:cursor-grabbing touch-none text-muted-foreground hover:text-foreground p-1" aria-label="Arrastar">
            ...
            {activeTab === "packs" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <h1 className="text-2xl font-heading font-bold text-foreground">Packs</h1>
                <div className="bg-card border border-border rounded-2xl p-6">
                ...
                <p className="text-xs text-muted-foreground">Arraste pelo ícone para reordenar dentro da categoria.</p>
                {packsByCategory.map((group: any) => (
                  <div key={group.category.id} className="space-y-2">
                    ...
                    {group.packs.length === 0 ? (
                      ...
                      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handlePackDragEnd(group.category.id)}>
                        <SortableContext items={group.packs.map((p: any) => p.id)} strategy={verticalListSortingStrategy}>
                          <div className="space-y-2">
                            {group.packs.map((p: any) => (
                              <SortableRow key={p.id} id={p.id}>
                                {(handleProps) => (
                                  <div className="bg-card border border-border rounded-xl px-4 py-3 ml-2">
                                    {editingPackId === p.id ? (
                                      <div className="space-y-3">
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
